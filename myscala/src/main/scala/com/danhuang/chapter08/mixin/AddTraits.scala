...
object AddTraits {
  def main(args: Array[String]): Unit = {
    //说明
    //1.创建Mysql4实例时，动态的混入DB4和File4
    //研究第一个问题，当我们创建一个动态混入对象时，其顺序是怎样的
    //1.Operate4...
    //2.Data4
    //3.DB4
    //4.File4
    //总结一句话，Scala在叠加特质的时候，会首先从后面的特质开始执行（即从左到右）
    val mysql = new MySQL4 with DB4 with File4
    println(mysql)
    println("--------------------------------------")

    //研究第2个问题，当我们执行一个动态混入对象的方法时，其执行的顺序是怎么样的
    //顺序是，1.从右到左开始开始执行，2.当执行到super时，是指的左边的特质.3.如果左边没有特质，则super就是指父特质
    //1.向文件
    //2.向数据库
    //3.插入数据 100
    mysql.insert(100)
...
trait Data4 extends Operate4{//特质，继承了Operate4
  ...
  override def insert(id: Int): Unit = {//实现/重写Operate4的insert
    println("插入数据="+id)
  }
}
trait DB4 extends Data4{//特质，继承Data4
  ...
  override def insert(id: Int): Unit = {//重写（不能叫实现了）Operate4的insert
    println("向数据库")
    super.insert(id)
  }
}
trait File4 extends Data4{//特质，继承Data4
  ...
  override def insert(id: Int): Unit = {//重写Data4的insert
    println("向文件")
//    super.insert(id) //调用了insert方法（难点），这里的super在动态混入时，不一定是父类
